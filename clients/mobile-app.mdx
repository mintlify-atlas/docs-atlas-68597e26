...
<Steps>
  ...
  <Step title="Start the dev server">
    Run one of the start scripts below. The default `npm start` points at a **local** Flask server (auto-detected from the LAN address Expo serves from). Use `start:prod` to point at production:

    ```bash
    # Local Flask server (auto-detected LAN address)
    npm start
...
<Note>
  Physical Android phones reach the host machine at `http://10.0.2.2:5000`; iOS simulators use `http://localhost:5000`. These are the `__DEV__` defaults already set in `lib/config.ts`. For a real phone on Expo Go, set `EXPO_PUBLIC_API_BASE` to your machine's LAN address (e.g. `http://192.168.1.20:5000`).
</Note>
...
`lib/config.ts` defaults to production in release builds, so `EXPO_PUBLIC_API_BASE` is the only knob you need — there is no risk of accidentally shipping a build that points at localhost.
...
- **Reads are cached.** Every successful response is stored via `lib/useQuery.ts`. Opening the app offline shows the last loaded data behind an "Offline" notice rather than a spinner into an error.
- **Writes are queued.** `lib/queue.ts` persists writes locally and replays them when connectivity returns. Only network-level failures (status 0, 502, 503, 504) are queued — a 400 surfaces immediately and rolls back the UI row so bugs don't hide.
...
<Warning>
  The app is **not yet on the App Store or Google Play**. Before the first submission you must run `npx eas init` (to fill in `extra.eas.projectId` for push tokens), set `ios.appleTeamId` and the Play service-account key in the `submit` block of `eas.json`, and bump `expo.version` in `app.json`.
</Warning>

## Push Notifications

`lib/push.ts` registers an Expo push token with `POST /api/v1/push/register`. The server stores it in the same `PushSubscription` table used by browser VAPID subscriptions, so every existing reminder reaches the phone without changes at the call site. Sign-out unregisters the token first.
...
├── lib/                    # api.ts, auth.tsx, useQuery.ts, queue.ts, push.ts…
├── theme/                  # Design tokens from ip-base.css
├── assets/                 # App icons (generated from static/icons/icon-512.png)
...
<Tip>
  The `theme/tokens.ts` file is a direct copy of the web palette from `static/css/ip-base.css`. If the web design tokens change, regenerate with `scripts/gen-icons.py` so the phone app stays visually in sync.
</Tip>
