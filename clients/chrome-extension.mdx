...
<CardGroup cols={2}>
  <Card title="Assignment Badge" icon="bell">
    The extension icon shows a live badge with the count of pending assignments fetched from `/extension/tasks`. The count refreshes automatically via the service worker background script.
  </Card>
  ...
  <Card title="Quick Actions" icon="bolt">
    Dismiss tasks, view your saved schedule, and check grades — all from the extension popup, backed by the same API endpoints the main web app uses.
  </Card>
</CardGroup>
...
The extension uses a dedicated set of endpoints on the IntelliPlan backend. All require a valid `Authorization: Bearer <token>` header obtained from the login endpoint.

<Accordion title="POST /extension/login">
  Authenticates the extension with an email and password, returning a bearer token for subsequent requests. This is called when the user submits the login form in `popup.html`.
  ...
  **Response**
  ...
  {
    "token": "<bearer-token>",
    "user": { "id": 42, "name": "Alex", "email": "student@example.edu" }
  }
  ```
</Accordion>

<Accordion title="GET /extension/tasks">
  Returns the list of pending assignments used to compute the toolbar badge count. The background service worker polls this endpoint on a schedule using the `alarms` permission.

  **Response**
  ```json
  {
    "tasks": [
      { "id": "abc123", "title": "Chapter 5 Reading", "due": "2025-06-10", "course": "AP Biology" }
    ],
    "count": 1
  }
  ```
...
<Accordion title="GET /extension/schedule">
  Returns the student's saved AI-generated study schedule for display in the popup's schedule tab.
</Accordion>

<Accordion title="GET /extension/grades">
  Returns per-course grade data pulled live from the connected school platform.
</Accordion>

<Accordion title="POST /extension/dismiss">
  Marks a task as dismissed/completed directly from the extension popup, mirroring the `/dismiss` endpoint on the main site.
